...
采用多级缓存：caffine + Redis + mysql 混合

- 查caffine，过期调用函数重建记忆
- 重建记忆时候先查Redis，有数据就使用Redis数据重建，没有数据从mysql中取数据重建

Caffeine可以理解为一个高效的key value缓存工具，比map的优势更多的在于可以轻松进行内存管理与过期淘汰等

#### 对话隔离实现机制

- 单例模式：给 AI 服务方法增加 `memoryId` 注解和参数，然后通过 `chatMemoryProvider` 为每个 appId 分配对话记忆

- 为每个 appId 创建一个 AiServices 来彻底隔离

  这里采用第二种，隔离程度比较高，需要修改 Factory 类为每个 appId 单独构建 chatMemory 和 AiServices 
...
controller主要用于处理前端请求对接等

service主要在于数据库 CRUD 操作、构建Querywapper、或者更复杂的逻辑chatToGenCode
